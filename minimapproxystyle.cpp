...
void MinimapProxyStyle::drawComplexControl(ComplexControl control,
    ...
    if (widget && control == QStyle::CC_ScrollBar && Settings::enabled()) {
        ...
        if (v.isValid()) {
            ...
            if (lineCount > 0 && lineCount <= Settings::lineCountThreshold()) {
...
bool MinimapProxyStyle::drawMinimap(const QStyleOptionComplex *option,
    ...
    int tab = helper->editor()->textDocument()->tabSettings().m_tabSize;
    int y = 0;
    int i = 0;
    qreal r = 0.0;
    bool codeFoldingVisible = helper->editor()->codeFoldingVisible();
    bool revisionsVisible = helper->editor()->revisionsVisible();
    bool folded = false;
    int revision = 0;
